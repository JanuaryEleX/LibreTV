...
async function fillAndSearchWithDouban(title) {
  ...
  // 检查是否有缓存，如果有则使用缓存，否则重新搜索
  const cacheKey = `searchResults_${safeTitle}`;
  ...
  if (cachedData) {
    try {
      ...
      if (cacheAge < cacheExpirationTime) {
        // 缓存有效，直接显示缓存结果
        if (typeof displayCachedResults === "function") {
          // 确保豆瓣资源API被选中
          if (
            typeof selectedAPIs !== "undefined" &&
            !selectedAPIs.includes("dbzy")
          ) {
            const doubanCheckbox = document.querySelector(
              'input[id="api_dbzy"]'
            );
            if (doubanCheckbox) {
          ...
          // 显示缓存结果
          displayCachedResults(parsedData, safeTitle);
